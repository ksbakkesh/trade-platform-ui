    ...
    <div className="space-y-4 pb-6 max-w-3xl">
          ...
          <form onSubmit={createUser} className="space-y-3">
            ...
            {email && password && (
              <div className="p-3 rounded-lg bg-surface-2 border border-white/10">
                <p className="text-muted text-[10px] mb-2 uppercase tracking-wider">Credentials to share with user</p>
                <div className="space-y-1.5 font-mono text-xs">
                  <div className="flex items-center justify-between">
                    <span className="text-muted">Email:</span>
                    <div className="flex items-center gap-2">
                      <span className="text-data">{email}</span>
                      <button type="button" onClick={() => copyToClipboard(email, 'email')}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-data">{showPassword ? password : '••••••••'}</span>
                      <button type="button" onClick={() => copyToClipboard(password, 'pwd')}
                        className="text-muted hover:text-accent transition-colors">
                        {copied === 'pwd' ? <Check size={12} className="text-accent" /> : <Copy size={12} />}
                      </button>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-data">localhost:3000/login</span>
                      <button type="button" onClick={() => copyToClipboard('http://localhost:3000/login', 'url')}
                        className="text-muted hover:text-accent transition-colors">
                        {copied === 'url' ? <Check size={12} className="text-accent" /> : <Copy size={12} />}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex gap-2 pt-1">
              <button type="submit" disabled={creating}
                ...
                {creating ? (
                  <><span className="w-3 h-3 border-2 border-bg/30 border-t-bg rounded-full animate-spin" />Creating...</>
                ) : 'Create Account'}
              </button>
      ...
      <div className="p-3 rounded-lg bg-surface-2 border border-white/5">
        <p className="text-muted text-xs leading-relaxed">
          ...
          Create an account here → share the email, password, and login URL with your client →
          they open the URL and sign in → they see only their own trades and positions.
          Each client needs their own Angel One credentials configured separately.
        </p>
      </div>
    </div>
  )
